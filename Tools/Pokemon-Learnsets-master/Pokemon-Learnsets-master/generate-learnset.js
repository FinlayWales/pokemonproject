        ...
        for (move in moveList[mon]) {//goes through each move
            for (valKey in moveList[mon][move]) {//goes through each method of obtaining
                ...
                if (moveVal.substr(0,1) == targetGen) {//puts move in array based on method of obtaining
                    ...
                    if      (moveType === 'L') {
                        if (newMoveList[mon].level[parseInt(moveVal.substr(2,3))] == undefined) {
                            newMoveList[mon].level[parseInt(moveVal.substr(2,3))] = move;
                        } else {
                            tempmove = newMoveList[mon].level[parseInt(moveVal.substr(2,3))];
                            newMoveList[mon].level[parseInt(moveVal.substr(2,3))] = [];
                            newMoveList[mon].level[parseInt(moveVal.substr(2,3))].push(tempmove);
                            newMoveList[mon].level[parseInt(moveVal.substr(2,3))].push(move);
                            newMoveList[mon].level[parseInt(moveVal.substr(2,3))] = [].concat.apply([],[].concat.apply([],newMoveList[mon].level[parseInt(moveVal.substr(2,3))]));
                        }
                    }
